...
main (int argc, char **argv)
{
  ...
  if (log_flag)
    fprintf(stderr, "Generating DH public key %d time(s) using public parameters %s", count, chosen_dhparam->name);

  double total_time = 0.0;
  for (long i = 0; i < count; i++) {
    void *dh = DH_key_gen_new(chosen_dhparam->name, chosen_dhparam->p, chosen_dhparam->g, chosen_dhparam->size, priv_key_size);

    const clock_t start_time = clock();
    int priv_key_size_actual = DH_key_gen_generate_keypair(dh, priv_key_size);
    const clock_t end_time = clock();
    total_time += ((double) (end_time - start_time)) / CLOCKS_PER_SEC;

    if (priv_key_size_actual < 0) {
      if (log_flag)
        fprintf(stderr, "DH public key generation error \n");
